...
cap = cv2.imread('/content/t3.jpg')   #to pass image path
...
def findObjects(outputs, img):
  ...
  for box in labeled_bbox_values: #iterate over total bboxes detected and separate them to a list according to classes
  ...
  #print(labeled_bbox_values)  #prints the  bbox lengths   
  #---------------> length of rider bbox,helmet bbox, license bbox is required to proceed 'Traffic violation Algorithm'
  ...
  for i in indices:
    ...
    #cv2.line(img, (70,0), (70, height), (0,255,0), 4)
    #cv2.line(img, (width-70,0), (width-70, height), (0,255,0), 4)
  ...
  cv2_imshow(img)
  #cv2.imwrite('/content/out4.jpg',img)
...
try:
  blob = cv2.dnn.blobFromImage(cap,1/255.0, (whT,whT),[0,0,0],1,crop=False) #cap >> img on video input
  net.setInput(blob)
...
def fetch_mock_DB(plate):
  dict_sample = {
          ...
          18: {'Name' : 'Calvin', 'Plate_no': 'kl23c5113'},
...
def detect_plate_ocr():
  for filename in os.listdir(path_violation_out):
    try:
      filepath = os.path.join(path_violation_out, filename) #/content/violation/123.jpg
